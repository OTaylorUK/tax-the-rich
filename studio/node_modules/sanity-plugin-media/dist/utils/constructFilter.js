...
var constructFilter = function (_a) {
    ...
    var documentAssetTypes = assetTypes.map(function (type) { return "sanity.".concat(type, "Asset"); });
    var baseFilter = (0, groq_1.default)(templateObject_1 || (templateObject_1 = __makeTemplateObject(["\n    _type in ", " && !(_id in path(\"drafts.**\"))\n  "], ["\n    _type in ", " && !(_id in path(\"drafts.**\"))\n  "])), JSON.stringify(documentAssetTypes));
    var searchFacetFragments = searchFacets.reduce(function (acc, facet) {
        ...
        if (facet.type === 'number') {
            ...
            var facetField = (currentModifier === null || currentModifier === void 0 ? void 0 : currentModifier.fieldModifier)
    ...
    var constructedQuery = __spreadArray(__spreadArray([
        // Base filter
        baseFilter
    ], __read((searchQuery
        ? [
            (0, groq_1.default)(templateObject_2 || (templateObject_2 = __makeTemplateObject(["[_id, altText, assetId, description, originalFilename, title, url] match '*", "*'"], ["[_id, altText, assetId, description, originalFilename, title, url] match '*", "*'"])), searchQuery.trim())
        ]
        : [])), false), __read(searchFacetFragments), false).join(' && ');
