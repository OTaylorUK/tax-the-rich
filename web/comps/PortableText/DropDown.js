...
function DropDown(prop) {
  ...
  // {value}
  const classList = 'm-1.5  font-bold text-3xl bg-transparent text-custom-tertiary underline text-center font-bold  uppercase w-full';
  ...
  useEffect(() => {
    ...
    data.map((obj, index) => {
			options.push(<option key={index} id={obj?._id}>{obj?.[selector]}</option>)
		})
  }
  const userSelect = (e) => {
